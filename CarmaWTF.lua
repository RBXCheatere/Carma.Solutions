...
function library:CreateWindow(name, size, hidebutton)
    ...
    list.Line.Name = "line"
    list.Line.ZIndex = 2
    list.Line.Size = UDim2.new(1, 0, 0, 1)
    list.Line.Position = UDim2.new(0, 0, 1, -1)
    ...
    list.BlackOutline.Size = UDim2.new(1, 4, 1, 4)
    list.BlackOutline.BorderSizePixel = 0
    list.BlackOutline.BackgroundColor3 = window.theme.outlinecolor2
    list.BlackOutline.Position = UDim2.new(0, -2, 0, -2)

    local size = game:GetService("TextService"):GetTextSize(list.name, 13, window.theme.font, Vector2.new(2000, 2000))
    list.Label = Instance.new("TextLabel", list.Main)
    list.Label.AnchorPoint = Vector2.new(0, 0.5)
    list.Label.Position = UDim2.new(0, 12, 0, -1)
    list.Label.Size = UDim2.new(0, math.clamp(size.X + 10, 0, list.Main.Size.X.Offset), 0, size.Y)
    list.Label.BackgroundTransparency = 1
    list.Label.BorderSizePixel = 0
    list.Label.ZIndex = 4
    list.Label.Text = list.name
    list.Label.TextColor3 = Color3.fromRGB(255, 255, 255)
    ...
    list.LabelBackFrame.Size = UDim2.new(1, 0, 0, 10)
    list.LabelBackFrame.BorderSizePixel = 0
    list.LabelBackFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    list.LabelBackFrame.Position = UDim2.new(0, 0, 1, 6)

    list.Items = Instance.new("ScrollingFrame", list.Main) 
    list.Items.Name = "items"
    list.Items.ZIndex = 2
    list.Items.ScrollBarThickness = 4
    list.Items.BackgroundTransparency = 1
    list.Items.Size = UDim2.new(1, 0, 1, -25)
    list.Items.Position = UDim2.new(0, 0, 0, 25)
    list.Items.CanvasSize = UDim2.new(0, 0, 0, 0)
    list.Items.ScrollingDirection = Enum.ScrollingDirection.Y

    list.ListLayout = Instance.new("UIListLayout", list.Items)
    list.ListLayout.SortOrder = Enum.SortOrder.LayoutOrder
    list.ListLayout.Padding = UDim.new(0, 4)
    ...
    function list:AddPlayer(Player)
        ...
        player.Main.Size = UDim2.new(1, 0, 0, 20)
        player.Main.BackgroundColor3 = window.theme.itemcolor

        table.insert(list.items, player)
        list.Items.CanvasSize = UDim2.new(0, 0, 0, #list.items * 24) -- Adjusted canvas size based on item count
        return player
    end
    ...
    function list:RemovePlayer(Player)
        ...
        if p then
            ...
            list.Items.CanvasSize = UDim2.new(0, 0, 0, #list.items * 24) -- Adjusted canvas size after removal
        end
    end

    -- Initial population of the list with existing players
    for _, player in ipairs(game.Players:GetPlayers()) do
        list:AddPlayer(player)
    end

    -- Connect to player added event to update the list dynamically
